...
export class EmailService {
  ...
  static enviarCorreoATodos(datosReporte: any): Promise<any> {
    return new Promise(async (resolve) => {
      try {
        ...
        const procesarCorreo = async (correo: string) => {
          try {
            ...
            errores++;
            console.error(`❌ Error enviando a ${correo}:`, (error as Error).message);
          } finally {
            completados++;
  ...
  static async enviarCorreoIndividual(correo: string, datosReporte: any) {
  ...
  static generarHTMLReporte(datos: any) {
    return `
    <!DOCTYPE html>
    <html>
    <head>
  ...
  static generarTextoReporte(datos: any) {
    return `
