...
evidenceRoutes.post(
  "/single", 
  UploadFactory.getEvidenciasUpload().single('file'), 
  UploadErrorHandler.handle, 
  EvidenceController.uploadSingle
);

// Subir múltiples evidencias (imágenes o PDFs)
evidenceRoutes.post(
  "/multiple", 
  UploadFactory.getEvidenciasUpload().array('files', 5), 
  UploadErrorHandler.handle, 
  EvidenceController.uploadMultiple
);
